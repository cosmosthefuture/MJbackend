...
use App\Http\Requests\Master\Withdraw\MasterWithdrawHistoryRequest;
use App\Http\Requests\Master\Withdraw\WithdrawRequestCreateRequest;

use Illuminate\Http\Request;
use App\Http\Services\Master\WithdrawService;
...
class WithdrawController extends ApiController
{
    ...
    public function masterWithdrawRequestList(MasterWithdrawHistoryRequest $request)
    {
        try {
            ...
            $res_data = $this->withdraw_service->getMasterWithdrawHistoryWithPagination(auth('api-master')->user()->id, $per_page, $page, with: $with);
            return $this->paginatedSuccessResponse($res_data, 200, 'Master Withdraw History Lists');
        } catch (\Exception $e) {
            logger()->error($e);
    ...
    public function create(WithdrawRequestCreateRequest $request)
    {
        try {
            ...
            $data = $this->withdraw_service->whereLatest('master_id', $master->id);
